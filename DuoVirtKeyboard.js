...
// ==/UserScript==
	...
	virtKeyboard = {
		...
		updatesupportedlangs: function(keys, isDL){
			...
			var secondlang=$("#text-input, #word-input").attr("lang")||virtKeyboard.mainlang;
			var namednodes={
				main:{
					name:"English",
		...
		updateLangs: function(update){
			if (!update) {update={};}
			for (var lcode in basekeys.supported_lang) {
				if (!basekeys[basekeys.supported_lang[lcode]]||update[basekeys.supported_lang[lcode]]){
					$.ajax({//get language layout
						type: "get",
						url: "http://192.168.2.36:8887/duo/keyboard." + basekeys.supported_lang[lcode] + ".json"
					}).done(function (json) {
		...
		virtKeyOnClick: function() {
			...
			$("#virt-keyboard").on("click", ".key", function (){
				...
				if($(this).hasClass("special")){
					...
					var keypressed = jQuery.Event({ "type":"keypress", "keyCode":keycode, "which":keycode, "shiftKey":shift, "key":keyname});
			...
			$(document).on("keydown", "textarea", function (keypressed) {
					...
					if (!(basekeys[input_lang]) || !basekeys[input_lang][keypressed.keyCode] || keypressed.altKey || keypressed.ctrlKey || keypressed.keyCode < 32) {
						return true;
					}
		...
		init: function(){
			/*
			$("#root > div > div > div > div > div > div > div > div > textarea").on("focus",function(){
				console.info("Textarea focused");
			});
			*/
			...
			$(document).on("focus", "textarea", function(){
				$("#virt-keyboard").removeClass("vrt-hidden");
			});
			$(document).on("focusout", "textarea", function(){
				if (!$("#virt-keyboard").hasClass("vrt-keep")){
					$("#virt-keyboard").addClass("vrt-hidden");
				}
			});
			...
			if (!oldkeys) {
				$.ajax({//get base keyboard layout
					type: "get",
					url: "https://192.168.2.36:8887/duo/keyboard.base.json"
				}).done(function (json) {
...
//			userId = virtKeyboard.getLoggedInUserId();
			virtKeyboard.mainlang = "en";
			virtKeyboard.init();
	/*
			$.ajax({//get user info
				type: "GET",
				url: "2016-04-13/users/" + userId,
				data: "{fields:username,courses,currentCourse}"
			}).done(function (json) {
				virtKeyboard.mainlang = json.currentCourse.fromLanguage;
				virtKeyboard.init();
			});
	*/	
		}
